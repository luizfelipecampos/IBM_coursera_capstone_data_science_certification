...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                step=1000, marks={0: '0', 2500: '2500', 5000: '5000',
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        counts = list(filtered_df['class'].value_counts().values)
        classes = filtered_df['class'].value_counts().index.to_list()
        plot_df = pd.DataFrame({'class': classes, 'count': counts})
        fig = px.pie(plot_df, values='count', names='class',
        title='Total Success Launches By Site')
        return fig
...
def get_scatter_chart(entered_site, entered_payload):
    filtered_df = spacex_df
    mask1 = filtered_df['Payload Mass (kg)'] >= entered_payload[0]
    mask2 = filtered_df['Payload Mass (kg)'] <= entered_payload[1]
